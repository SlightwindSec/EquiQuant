...
class Qwen3MoeSparseSplitMoeBlock(nn.Module):
...
def convert_experts_to_mlp(
    original_moe_block,
    config,
) -> Qwen3MoeSparseSplitMoeBlock:
    new_moe_block = Qwen3MoeSparseSplitMoeBlock(config)
...
def convert_qwen3_moe(model):
    logger.info("Converting experts to mlp for qwen3_moe...")
