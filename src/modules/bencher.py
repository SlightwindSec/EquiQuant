...
class AisBencher:
    ...
    def __init__(
        self, eval_config, server_config, quantized_model_path, current_run_dir, run_id
    ):
        """
        Args:
        ...
        self.disable_thinking = bool(
            self.eval_config.get("disable_qwen_thinking", False)
        )
        ...
        self.cleanup_generated_config = self.ais_config.get(
            "cleanup_model_config", True
        )
        ...
        self.default_metric_keys = self.ais_config.get(
            "default_metric_keys", ["final_accuracy", "accuracy", "score"]
        )

        # vLLM service 信息
        self.served_model_name = self._resolve_served_model_name()
        self.host_ip = self.ais_config.get("host_ip") or self.server_config.get(
            "host", "localhost"
        )
        self.host_port = self.ais_config.get("host_port") or self.server_config.get(
            "port"
        )
    ...
    def run(self, early_stop_fn=None) -> Dict[str, float]:
        ...
        logger.info(
            f"Starting AISBench evaluation for {len(self.datasets)} dataset(s)..."
        )

        try:
            for alias, dataset_cfg in self.datasets.items():
                logger.info(f"[AISBench] Dataset: {alias}")
                request_rate = dataset_cfg.get(
                ...
                try:
                    ...
                    logger.error(
                        f"Could not generate ais_bench model config for {alias}: {exc}"
                    )
                    results[alias] = 0.0
                    continue
                ...
                # 尝试从输出文件中解析结果
                accuracy, result_data = self._parse_results_from_files(
                    stdout + "\n" + stderr, alias, dataset_cfg
                )
                if accuracy is None:
                    # 如果文件解析失败，回退到日志解析
                    accuracy = self._parse_accuracy(
                        stdout + "\n" + stderr, alias, dataset_cfg
                    )
                    logger.warning(
                        f"Failed to parse results from files for {alias}, using log parsing fallback."
                    )
    ...
    def _prepare_model_config_handle(self):
        ...
        if explicit_dir:
            ...
            self.model_config_dir = (
                base_dir / "benchmark" / "configs" / "models" / subdir
            )

        if not self.model_config_dir.exists():
            raise FileNotFoundError(
                f"Model config directory not found: {self.model_config_dir}"
            )
        ...
        logger.info(
            f"AISBench model config will be written to: {self.model_config_path}"
        )
    ...
    def _write_model_config(self, request_rate: Optional[float] = None):
        ...
        generation_kwargs = copy.deepcopy(
            self.ais_config.get("generation_kwargs", generation_defaults)
        )
        ...
        request_rate_value = (
            request_rate if request_rate is not None else self.default_request_rate
        )
    ...
    def _cleanup_model_config(self):
        if (
            self.cleanup_generated_config
            and self.model_config_path
            and self.model_config_path.exists()
        ):
            try:
                self.model_config_path.unlink()
                logger.debug(
                    f"Removed temporary AISBench model config: {self.model_config_path}"
                )
            except Exception as exc:
                logger.warning(
                    f"Failed to remove AISBench model config {self.model_config_path}: {exc}"
                )
    ...
    def _write_log(self, dataset_alias: str, cmd: str, stdout: str, stderr: str) -> str:
        log_dir = self.ais_config.get("log_dir") or os.path.join(
            self.current_run_dir, "aisbench_logs"
        )
    ...
    def _parse_results_from_files(
        self, logs: str, dataset_alias: str, dataset_cfg: Dict
        ...
        if summary_data:
            ...
            if accuracy is not None:
                ...
                result_data["results"] = self._parse_results_json(
                    output_path, dataset_alias, dataset_cfg
                )
                result_data["predictions"] = self._parse_predictions_json(
                    output_path, dataset_alias, dataset_cfg
                )
    ...
    def _parse_summary_csv(
        self, output_path: str, dataset_alias: str, dataset_cfg: Dict
    ) -> Optional[Dict]:
        """
        解析 summary CSV 文件。
        ...
        try:
            with open(csv_file, "r", encoding="utf-8") as f:
                ...
                for row in rows:
                    ...
                    # 检查数据集名称是否匹配（支持部分匹配）
                    if (
                        dataset_name.lower() in row_dataset
                        or row_dataset in dataset_name.lower()
                        or dataset_alias.lower() in row_dataset
                    ):
                        matched_row = row
                        break
                ...
                if not matched_row:
                    logger.warning(
                        f"No matching dataset found in summary CSV for {dataset_alias}"
                    )
                    return None
                ...
                if accuracy is None:
                    logger.warning(
                        f"Could not extract accuracy from summary CSV row: {matched_row}"
                    )
                    return None
    ...
    def _parse_results_json(
        ...
        for json_file in json_files:
            ...
            if (
                dataset_name.lower() in file_name
                or file_name in dataset_name.lower()
                or dataset_alias.lower() in file_name
            ):
    ...
    def _parse_predictions_json(
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse predictions JSON file {matched_file}: {exc}"
            )
            return None
    ...
    def _save_result_data(self, dataset_alias: str, result_data: Dict):
        ...
        save_dir = os.path.join(
            self.current_run_dir, "aisbench_results", f"trial{self.run_id:03d}"
        )
            ...
            try:
                ...
                if source_file and os.path.exists(source_file):
                    ...
                    with open(predictions_file, "w", encoding="utf-8") as f:
                        json.dump(
                            result_data["predictions"], f, indent=2, ensure_ascii=False
                        )
                    logger.debug(f"Saved predictions data to {predictions_file}")
            except Exception as exc:
    ...
    def _parse_accuracy(
        ...
        logger.warning(
            f"Could not parse accuracy from AISBench logs for {dataset_alias}. Defaulting to 0.0."
        )
        return 0.0
    ...
    def _resolve_served_model_name(self) -> str:
        ...
        for key in ("served-model-name", "served_model_name"):
            if key in args and args[key]:
                return args[key]
        raise KeyError(
            "vllm_server.args must provide 'served-model-name' for AISBench."
        )
