...
class ModelInspector:
    ...
    def get_layers_by_pattern(model_path, pattern):
        ...
        try:
            ...
            if "{i}" not in pattern:
                logger.warning(f"Layer pattern '{pattern}' does not contain '{{i}}'. Returning as is.")
                return [pattern]

            generated_layers = [pattern.format(i=i) for i in range(num_layers)]
            logger.info(f"Generated {len(generated_layers)} layer names for fallback strategy.")
