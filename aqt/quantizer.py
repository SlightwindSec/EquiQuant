...
class Quantizer:
    ...
    def _find_params_minmax(self, weight: torch.Tensor) -> None:
        ...
        if self.quant_sym:
            ...
            self._zero_point = torch.zeros(
                out_features, 1, dtype=torch.int32, device=weight.device
            )
        else:
            self._scale = (w_max - w_min) / (self.qmax - self.qmin)
            self._zero_point = torch.round(self.qmin - w_min / self._scale).to(
                torch.int32
            )
    ...
    def quantize_weight(
        ...
        if self._scale is None or self._zero_point is None:
            raise RuntimeError(
                "Quantization parameters not found. Call find_params first."
            )
    ...
    def dequantize_weight(
        ...
        dequant_weight = (
            quant_weight.to(scale.dtype) - zero_point.to(scale.dtype)
        ) * scale
